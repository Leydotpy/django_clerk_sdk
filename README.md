...
## Important configuration keys
 ...
 ```python 
 from django_clerk_sdk.defaults import *
 ```
 ...
 ```python
 from django_clerk_sdk import defaults
 AUTH_USER_MODEL = defaults.AUTH_USER_MODEL or AUTH_USER_MODEL = "users.ClerkUser" 
 ```
 (requires 'django_clerk_sdk.users' in INSTALLED_APPS)
...
CLERK_AUTH_PARTIES = ["..."]
CLERK_AUTH_METRICS_COLLECTOR = "path.to.import" # Optional. defaults to _InMemoryCollector()
```

The CLERK_AUTH_METRICS COLLECTOR setting assumes you have created a metric collector which inherits from the `MetricsCollectorProtocol` class exposed in the `django_clerk_sdk.core.auth.clerk.authentication` module that exposes a `.increment()` method or you have implemented a custom implementation that yields same.

```python
from django_clerk_sdk.core.auth.clerk.authentication import MetricsCollectorProtocol

class MyMetricCollector(MetricsCollectorProtocol):
...
class MyMetricCollector:
    ...
    def increment(self, name: str, amount: int = 1, tags: dict | None = None):
        # code body
        ...
...
Prefer the module-level sdk singleton (exported from django_clerk_sdk.core.auth.clerk) for runtime operations in views/middleware. It lazily constructs a client and is thread-safe. The SDK will try to map users using clerk_user_id if your User model exposes that field; otherwise it falls back to primary email.
The package ships a simple user model (ClerkUser) and migrations. If you use your own custom user model, ensure it exposes fields expected by the SDK (username/email/image/last_active_at/clerk_user_id/image) or adapt the SDK mapping. Use the provided DRF authentication classes to get caching and metrics hooks; configure CLERK_AUTH_METRICS_COLLECTOR to collect counters. 
Development Source is under [src/django_clerk_sdk/.]
